...
//			                    add sản phẩm khuyến mãi
				...
				if (km != null) {
					...
					boolean kmmoi = kmDao.updateKhuyenMai(tenkm, phanTramText, ngaybdsql, ngayktsql, stat,
							selectedRowsValues.size(), maKM);
...
// Kiểm tra dữ liệu	
	private KhuyenMai valiData() {
		...
		if (ngaybd == null
				|| ngaybd.toInstant().atZone(ZoneId.systemDefault()).toLocalDate().isBefore(LocalDate.now())) {
		...
		return new KhuyenMai(txtMaKM.getText().trim(), tenkm, (int) cboPhanTram.getSelectedItem(), ngaybd, ngaykt, true,
				0);
	}
	...
	public void docDuLieu() {
		...
		for (KhuyenMai x : list) {
			...
			tablemodel1.addRow(new Object[] { x.getMaKhuyenMai(), x.getTenKhuyenMai(), x.getPhanTram(),
	...
	public void docDuLieuSP() {
		...
		for (sanPham x : list) {
			...
			tablemodel.addRow(new Object[] { x.getMaSP(), x.getTenSP(),
					x.getKhuyenMai().getMaKhuyenMai() == null ? false : true });
		}
	}
